...
module @softmax {
  func.func @test(%arg0: memref<32x1024xf32>) -> memref<32x1024xf32> {
    ...
    scf.parallel (%arg1) = (%c0_1) to (%c32) step (%c1) {
      scf.parallel (%arg2) = (%c0_2) to (%c64) step (%c1_3) {
        %0 = arith.addi %c0, %arg2 : index
        %1 = vector.transfer_read %arg0[%arg1, %0], %cst_0 {in_bounds = [true]} : memref<32x1024xf32>, vector<16xf32>
        ...
        vector.transfer_write %10, %alloc[%arg1, %11] {in_bounds = [true]} : vector<16xf32>, memref<32x1024xf32>
        scf.yield
      }
      scf.yield
    }
